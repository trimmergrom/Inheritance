...
void main()
{
	...
	int n = 0;//Ðàçìåð ôàéëà â ñòðîêàõ
	if (fin.is_open())
	{
		//1) Ñ÷èòàåì ðàçìåð ôàéëà:
		std::string buffer;
		while (!fin.eof())
		{
		...
		//2) Âûäåëÿåì ïàìÿòü ïîä ìàññèâ óêàçàòåëåé:
		group = new Human * [n] {};
		cout << fin.tellg() << endl;

		//3) Âîçâðàùàåìñÿ â íà÷àëî ôàéëà, äëÿ òîãî, ÷òîáû ïðî÷èòàòü åãî:
		fin.clear();
		fin.seekg(0);
		cout << fin.tellg() << endl;

		//4) ×èòàåì ôàéë, è çàãðóæàåì åãî ñòðîêè â ïàìÿòü:
		std::string type;
		for (int i = 0; i < n; i++)
		{
			cout << fin.tellg() << endl;
